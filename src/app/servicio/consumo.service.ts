  ...
  guardarConsumo(unConsumo: Consumo) {
    ...
    if (res[0] == 'ok') {
      ...
      unConsumo.items.forEach(unItem => {
        ...
        this.item.insertItems = this.item.insertItems + `INSERT INTO ITEMS_CONSUMO (id_consumo, total, codigo, nombre, cantidad, precio_venta, precio_costo, ganancia) values (${unItem.id_consumo},${unItem.total},'${unItem.codigo}','${unItem.nombre}',${unItem.cantidad},${unItem.precio_venta}, ${unItem.precio_costo}, ${unItem.ganancia});`;
